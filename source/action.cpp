...
int flash_from_file(const char *filename, int raw) {
    ...
    if (cApp.Warning("Flashing is complete, reboot the console?") == TRUE) {
        xenon_smc_power_reboot();
    }
...
void DumpNand(char * dest) {
    ...
    if (cApp.Warning("Warning dump nand ???") == FALSE) {
        return;
    }
    ...
    if (f) {
        ...
        for (unsigned int i = 0; i < nand_size; i += sfc.block_sz) {
            ...
            if (buffer_len != fwrite(buff, 1, buffer_len, f)) {
                cApp.Alert("Error dumping nand !!! please check disk space !!");
            }
            pct = ((float) i / (float) (nand_size));
            cApp.SetProgressValue(pct);
        }
...
void ActionFlashNand(const char * filename) {
    if (cApp.Warning("Continue ???") == FALSE) {
        return;
    }
...
void ActionLaunchFile(const char * filename) {
    ...
    if (FileIsElf(filename)) {
        if (cApp.Warning("Continue with launch elf ???") == FALSE) {
            return;
        }
    ...
    if (FileIsNand(filename)) {
        if (cApp.Warning("Flash this nand file ???") == FALSE) {
            return;
        }
